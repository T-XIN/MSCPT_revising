...
do
    ...
    do 
        for seed in $seeds
        do
            echo "Running with seed=$seed and num_shot=$num_shot base_model=$base_model"
            ...
            --dataset_name "Lung" --dataset "my_data" --model_name ${model_names[${base_model}]} --total_epochs ${epochs[${base_model}]} --n_tpro $n_tpro --n_vpro $n_tpro \
